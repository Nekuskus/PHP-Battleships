...
<body>
    ...
    class Battleships {
        ...
        public int $y_width;
        public int $z_width;
        function __construct(int $x_width = 10, int $y_width = 10, int $z_width = 3, array $ships = [4 => 1, 3 => 2, 2 => 3, 1 => 4]) {
            $this->x_width = $x_width;
            $this->y_width = $y_width;
            $this->z_width = $z_width;
            $this->inner = [];

            for ($z=0; $z < $this->z_width; $z++) {
                array_push($this->inner, []);
                for ($y=0; $y < $y_width; $y++) {
                    array_push($this->inner[$z], []);
                    for ($x=0; $x < $x_width; $x++) { 
                        array_push($this->inner[$z][$y], Field::Empty);
                    }
                }
            }
            ...
            while(count($ships) > 0) {
                ...
                while ($ships[$first_idx] > 0) {
                    $num = rand(0, $z_width * $x_width * $y_width - 1);
                    $x = $num % $x_width;
                    $y = intdiv($num, $x_width) % $y_width;
                    $z = intdiv($num,($x_width * $y_width));
                    $curship = [];
                    if(!$this->place($z, $y, $x)) {
                        $fail_count++;
                        ...
                        do {
                            ...
                            $x = $num % $x_width;
                            $y = intdiv($num, $x_width) % $y_width;
                            $z = $num / ($x_width * $y_width);
                            $fail_count++;
                            if($fail_count == $safeguard) {
                                throw new DomainException();
                            }
                        } while(!$this->place($z, $y, $x) && $fail_count != $safeguard);
                    }

                    array_push($curship, [$z, $y, $x]);

                    while (count($curship) < $first_idx) {
                        $next_guess = [];
                        if($z > 0 &&!in_array([$z-1, $y, $x], $curship, true)) {
                            array_push($next_guess, [$z-1, $y, $x]);
                        }
                        if($y > 0 && !in_array([$z, $y-1, $x], $curship, true)) {
                            array_push($next_guess, [$z, $y-1, $x]);
                        }
                        if($y < $y_width-1 && !in_array([$z, $y+1, $x], $curship, true)) {
                            array_push($next_guess, [$z, $y+1, $x]);
                        }
                        if($x > 0 && !in_array([$z, $y, $x-1], $curship, true)) {
                            array_push($next_guess, [$z, $y, $x-1]);
                        }
                        if($x < $x_width-1 && !in_array([$z, $y, $x+1], $curship, true)) {
                            array_push($next_guess, [$z, $y, $x+1]);
                        }
                        if($z < $z_width - 1 && !in_array([$z+1, $y, $x], $curship, true)) {
                            array_push($next_guess, [$z+1, $y, $x]);
                        }
                        ...
                        while(!$this->place($next[0], $next[1], $next[2], $curship)) {
                            $fail_count++;
                            if($fail_count == $safeguard) {
                                throw new DomainException();
                            }
                        ...
                        array_push($curship, [$next[0], $next[1], $next[2]]);
                        $z = $next[0];
                        $y = $next[1];
                        $x = $next[2];
                    }
            ...
            $found = 0;
            for ($z_idx=0; $z_idx < $z_width; $z_idx++) { 
                for ($y_idx=0; $y_idx < $y_width; $y_idx++) { 
                    for ($x_idx=0; $x_idx < $x_width; $x_idx++) {
                        if($this->inner[$z_idx][$y_idx][$x_idx] == Field::Ship) $found += 1;
                    }
                }
            }

            if($found != $field_count) {
                throw new DomainException(); // No fallback, means an algorithmic error occured and should be fixed
            }
        }
        ...
        function place(int $z, int $y, int $x, array $excl = []) : bool {
            if($this->checkValidForPlace($z, $y, $x, $excl)) {
                $this->inner[$z][$y][$x] = Field::Ship;
                return true;
            }
            return false;
        }

        function checkValidForPlace(int $z, int $y, int $x, array $excl = []) : bool {
            if($z > 0) {
                if($y > 0) {
                    if ($x > 0) {
                        if($this->inner[$z-1][$y-1][$x-1] !== Field::Empty && !in_array([$z-1, $y-1, $x-1], $excl, true)) {
                            return false;
                        }
                    }
    
                    if($this->inner[$z-1][$y-1][$x] !== Field::Empty && !in_array([$z-1, $y-1, $x], $excl, true)) {
                        return false;
                    }
                ...
                if ($x > 0) {
                    if($this->inner[$z-1][$y][$x-1] !== Field::Empty && !in_array([$z-1, $y, $x-1], $excl, true)) {
                        return false;
                    }
                }
    
                if($this->inner[$z-1][$y][$x] !== Field::Empty && !in_array([$z-1, $y, $x], $excl, true)) {
                    return false;
                }
            ...
            if($y > 0) {
                if ($x > 0) {
                    if($this->inner[$z][$y-1][$x-1] !== Field::Empty && !in_array([$z, $y-1, $x-1], $excl, true)) {
                        return false;
                    }
                }

                if($this->inner[$z][$y-1][$x] !== Field::Empty && !in_array([$z, $y-1, $x], $excl, true)) {
                    return false;
                }
            ...
            if ($x > 0) {
                if($this->inner[$z][$y][$x-1] !== Field::Empty && !in_array([$z, $y, $x-1], $excl, true)) {
                    return false;
                }
            }

            if($this->inner[$z][$y][$x] !== Field::Empty && !in_array([$z, $y, $x], $excl, true)) {
                return false;
            }
        ...
        static function unserialize(string $s): Battleships {
            ...
            for ($z=0; $z < $z_width; $z++) { 
                for ($y=0; $y < $y_width; $y++) {
                    for ($x=0; $x < $x_width; $x++) { 
                        $ret->inner[$z][$y][$x] = Field::from($chars[$total]);
    ...
    function settings_panel() {
        ...
        echo "<div><label for='z-displ'>Wyświetlanie z</label> <input name='z-displ' type='number' min='1' step='1' value='1'></div><br>";
        ...
        echo "<button type='submit'>Generuj</button><br>";
        echo "<p>Nawigacja warstw: <kbd>z</kbd>, <kbd>x</kbd></p><br><br>";
        echo "</form>";
        echo "</div>";
    }
    ...
    function create_game(string $next) {
        ...
        if(isset($_GET['z-displ'])) {
            $z_displ = intval($_GET['z-displ']) ?? 1;
        } else {
            $z_displ = 1;
        }
        ...
        try {
            ...
            let GAME_JSON = {$json};</script>";
            if($next === 'play') {
                render_play($game, $z_displ);
            } else {
                render_displ($game, $z_displ);
            }
        } catch (\Throwable $th) {
            echo "<div id='game-wrapper'><div id='game'>";
            echo "Generacja planszy nieudana, spróbuj ponownie (auto: za 3s), lub popraw swoje dane aby umożliwić stworzenie <i>poprawnej</i> planszy do gry w statki.";
    ...
    function render_play(Battleships $game, $z_displ = 1) {
        $x1 = $game->x_width + 1;
        $y1 = $game->y_width + 1;

        $z_anchor = intval(ceil($game->z_width / 2));
        echo "<script>
        ...
        echo "<div id='game-wrapper'><div id='game'>";
        for ($z=0; $z < $game->z_width; $z++) { 
            $opacity = abs($z_anchor - ($z+1)) <= $z_displ ? number_format(1-abs($z+1-$z_anchor) / $game->z_width, 2) : 0;
            $tx = ($z_anchor - ($z+1)) * 2.5;
            $ty = - $tx;
            $coord_visibility = ($tx == 0 ? 'visible' : 'hidden');
            $outline = ($tx == 0 ? '' : "outline: none;");
            $events = ($tx == 0 ? '' : 'no-events');
            echo "<div class='layer $events' style='opacity: {$opacity}; transform: translate({$tx}%, {$ty}%);'>";
            echo "<div class='cell pad empty' style='visibility: {$coord_visibility};'></div>";
            for ($x=0; $x < $game->x_width; $x++) { 
                echo "<div class='cell label' style='visibility: {$coord_visibility};'><span class='coord'>".($x+1)."</span></div>";
            }
        ...
        echo "<script>let CUR_LAYER = {$z_anchor};let Z_DISPL = {$z_displ};</script>";
        echo "</div></div>";
    }

    function render_displ(Battleships $game, $z_displ = 1) {
        $x1 = $game->x_width + 1;
        $y1 = $game->y_width + 1;
        ...
        echo "<div id='game-wrapper'><div id='game'>";
        for ($z=0; $z < $game->z_width; $z++) { 
            $opacity = abs($z_anchor - ($z+1)) <= $z_displ ? number_format(1 * 1-abs($z+1-$z_anchor) / $game->z_width, 2) : 0;
            $tx = ($z_anchor - ($z+1)) * 2.5;
            $ty = - $tx;
            echo "<div class='layer' style='opacity: {$opacity}; transform: translate({$tx}%, {$ty}%);'>";
            $coord_visibility = ($tx == 0 ? 'visible' : 'hidden');
            $outline = ($tx == 0 ? '' : "outline: none;");
            echo "<div class='cell pad empty' style='visibility: {$coord_visibility};'></div>";
            for ($x=0; $x < $game->x_width; $x++) {
            ...
            for($y=0; $y < $game->y_width; $y++) {
                echo "<div class='cell label' style='visibility: {$coord_visibility};'><span class='coord'>".getNameFromNumber($y)."</span></div>";
                for ($x=0; $x < $game->x_width; $x++) { 
                    switch($game->inner[$z][$y][$x]) {
                        case Field::Ship:
                            echo "<div z='{$z}' y='{$y}' x='{$x}' class='cell field ship' style='$outline'></div>";
                            break;
                        case Field::Shot:
